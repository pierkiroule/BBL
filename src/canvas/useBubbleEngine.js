...
function drawStroke(
  ...
  // ✅ Opacité calculée UNE FOIS ici
  const opacity = computeAlpha( {
    isGhost,
    presence,
...
for (let s = 0; s < symmetry; s += 1) {
  ...
  // ✅ Source unique de vérité pour l’opacité
...
function eraseHard(strokes, eraser) {
  const r2 = eraser.size * eraser.size;
  const ePoints = eraser.points || [];

  if (!ePoints.length) return strokes;

  return strokes
    .map(stroke => {
      if (stroke.tool === 'eraser') return stroke;

      const kept = stroke.points.filter(p => {
        for (let i = 0; i < ePoints.length; i++) {
          const dx = p.x - ePoints[i].x;
          const dy = p.y - ePoints[i].y;
...
const undoStackRef = useRef([]); // redo stack en pratique
const bubbleBreathRef = useRef(1);
...
[drawCtxRef.current,
loopCtxRef.current].forEach((ctx) => {
if (ctx) {
ctx.lineCap = 'round';
ctx.lineJoin = 'round';
}
...
dprRef.current = dpr;
[drawingRef.current,
loopRef.current].forEach((canvas) => {
if (canvas) {
...
(ctx, time, res, sizeOverride) => {
  ...
  // --- AUDIO (source unique) ---
  const bass = res?.bass ?? 0;
  const mid = res?.mid ?? 0;
  const treble = res?.treble ?? 0;

  const audioEnergy = Math.min(1, bass * 1.2 + mid * 0.6);
...
const envTarget = audioEnergy;
const env = audioEnvelopeRef.current;

audioEnvelopeRef.current =
  envTarget > env
    ? env + (envTarget - env) * attack
    : env + (envTarget - env) * release;

// --- RESPIRATION ---
const target = 1 + audioEnvelopeRef.current * 0.12;
bubbleBreathRef.current += (target - bubbleBreathRef.current) * 0.1;
const breathe = bubbleBreathRef.current;
...
...p, t: Math.min(clampDuration, p.t * ratio)
})),
});
...
const size = displaySizeRef.current || 640;
const dpr = 1; // export = pixels réels
...
const drawFrameForExport = () => {
  ...
  renderFrame(ctx, elapsed, resonanceRef.current, size);
...
const setCameraMode = useCallback((enabled) => {
  cameraModeRef.current = !!enabled;

  // optionnel : geler le temps
  if (enabled) {
    setLoopPause(true);
  }
}, [setLoopPause]);

const stepCamera = useCallback((dx = 0, dy = 0, dz = 0) => {
  if (!cameraModeRef.current) return;

  cameraRef.current.x += dx;
  cameraRef.current.y += dy;
  cameraRef.current.zoom = clampZoom(
    cameraRef.current.zoom + dz
  );
}, []);
...
canRedo: () => undoStackRef.current.length > 0,
getSessionData,
loadSessionData,
};
}
